...
aqli_gadm2_collapse <- aqli_raster_brick_light %>%
  ...
  dplyr::rename(objectid_gadm2 = colormap_objectid, 
                pol_col_name_vec[i] = avg_pm2.5_pollution)
...
for(i in 1:length(collapsed_gadm2_files_list)){
  if(i == 1){
    ...
    aqli_gadm2_collapse_master <- aqli_gadm2_collapse_master %>% 
      ...
      dplyr::select(objectid_gadm2, iso_alpha3, country, name_1, name_2, population,
             whostandard, natstandard, pol_col_name_vec[1:(i-1)],
             avg_pm2.5_pollution)
...
# gadm2: compiling all gadm2 yearly datasets into a final gadm2 format to be shared with VIT
