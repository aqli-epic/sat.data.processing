...
# Politically Sensitive Regions (final step)

```{r eval=FALSE, include=FALSE}
#> create a India point of view gadm2 file---------------------------

# read in the finalized files for 2023 shared with ViT in April, 2023 (if running the pipeline, from scratch - use the)

aqli_gadm2_collapse_master_finalized <- read_csv("./ar.2023.update.using.2021.pol.data/data/output/[finalizedApr2023]gadm2_aqli2021_vit.csv")

aqli_gadm1_collapse_from_gadm2_finalized <-
  read_csv("./ar.2023.update.using.2021.pol.data/data/output/[finalizedApr2023]gadm1_aqli2021_vit.csv")

aqli_gadm0_collapse_from_gadm2_finalized <- read_csv("./ar.2023.update.using.2021.pol.data/data/output/[finalizedApr2023]gadm0_aqli2021_vit.csv")

# gilgit baltistan
gilgit_baltistan <- aqli_gadm2_collapse_master_finalized %>%
  filter(name0 == "Pakistan", name1 == "Gilgit Baltistan") %>%
  mutate(name0 == "India") 

# Azad Kashmir
azad_kashmir <- aqli_gadm2_collapse_master_finalized %>%
  filter(name0 == "India", name1 == "Azad Kashmir") %>%
  mutate(name0 == "India")

# India politically sensitve regions gadm2 file
aqli_gadm2_collapse_master_finalized_india_pov <- aqli_gadm2_collapse_master_finalized %>%
  filter(name1 %notin% c("Azad Kashmir", "Gilgit Baltistan")) %>%
  bind_rows(gilgit_baltistan) %>%
  bind_rows(azad_kashmir)

#> create China point of view gadm2 file----------------------------

# arunachal pradesh, gadm1
andhra_pradesh <- aqli_gadm2_collapse_master_finalized %>%
  filter(name0 == "India", name1 == "Arunachal Pradesh") %>%
  mutate(name0 == "China")

# taiwan, gadm0 shift to "China"
taiwan_to_china <- aqli_gadm2_collapse_master_finalized %>%
  filter(name0 == "Taiwan") %>%
  mutate(name0 == "China")

# hong kong
hong_kong <- aqli_gadm2_collapse_master_finalized %>%
  filter(name1 == "Hong Kong") %>%
  mutate(name0 == "China")

# Macau
macau <- aqli_gadm2_collapse_master_finalized %>%
  filter(name1 == "Macau") %>%
  mutate(name0 == "China")

# China politcially sensitive regions file
aqli_gadm2_collapse_master_finalized_china_pov <- aqli_gadm2_collapse_master_finalized %>%
  filter(name1 %notin%  "Andhra Pradesh", name0 %notin% "Taiwan", )
