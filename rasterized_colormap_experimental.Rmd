...
# regions that we don't expect to be missing according to the object ids (this will give us the 83 regions that we don't expect to be missing).
...
aqli_gadm2_collapse_master %>%
  readr::write_csv(str_c(ssd_location_collapsed_gadm2_data_path, "master_global_allyears_gadm2_non_geom_April022023.csv"))
...
aqli_gadm2_collapse_master_with_geom %>%
  sf::write_sf(str_c(ssd_location_collapsed_gadm2_data_path, "master_global_allyears_gadm2_with_geom_April022023.shp"))
...
aqli_gadm0_collapse_from_gadm2 %>%
  readr::write_csv(str_c(ssd_location_collapsed_gadm0_data_path, "master_global_allyears_gadm0_non_geom_April022023.csv"))
...
aqli_gadm0_collapse_from_gadm2_with_geom %>%
  sf::st_write(str_c(ssd_location_collapsed_gadm0_data_path, "master_global_allyears_gadm0_with_geom_April022023_.shp"), delete_layer = TRUE)
...
aqli_gadm1_collapse_from_gadm2 %>%
  readr::write_csv(str_c(ssd_location_collapsed_gadm1_data_path, "master_global_allyears_gadm1_non_geom_April022023.csv"))
...
aqli_gadm1_collapse_from_gadm2_with_geom %>%
  sf::st_write(str_c(ssd_location_collapsed_gadm1_data_path, "master_global_allyears_gadm1_with_geom_April022023.shp"), delete_layer = TRUE)
...
# gilgit baltistan
gilgit_baltistan <- gadm2_aqli_2021_with_added_regions %>%
  filter(country == "Pakistan", name_1 == "Gilgit Baltistan") %>%
  mutate(country == "India") 

# Azad Kashmir
azad_kashmir <- gadm2_aqli_2021_with_added_regions %>%
  filter(country == "India", name_1 == "Azad Kashmir") %>%
  mutate(country == "India")

# India politically sensitve regions gadm2 file
gadm2_aqli_2021_with_added_regions %>%
  filter(name_1 %notin% c("Azad Kashmir", "Gilgit Baltistan")) %>%
  bind_rows(gilgit_baltistan) %>%
  bind_rows(azad_kashmir)
