...
# 	filename = "./experimentation/colormap_rasterized_0.01_jan2023.tif",
# 	format = "GTiff", overwrite = TRUE)
#---

polygon_cells <- raster::raster("./experimentation/colormap_rasterized_0.01_jan2023.tif")
...
for (i in 1:length(pol_data_list)){
  ...
  # cur_pol_file_name <- "V5GL03.HybridPM25-NoDust-NoSeaSalt.NoLandMask.Global.201801-201812"
  
  # cur pollution file year
  cur_pol_file_year <- stringr::str_extract(str_extract(cur_pol_file_name, "(\\d+)-(\\d+)"), "....")
  # cur_pol_file_year <- 2018
  ...
  # pollution_dataset <- raster::raster("./experimentation/others/V5GL03.HybridPM25-NoDust-NoSeaSalt.NoLandMask.Global.201801-201812.nc")
...
pop_raw_landscan_crop_pol <- foster::matchResolution(pop_raw_landscan_crop_pol, pollution_dataset)

pol_0.01_region_in_landscan_pop_res <- pollution_dataset
...
# temp
aqli_gadm2_collapse_master <- read_csv(str_c(ssd_location_collapsed_gadm2_data_path, "master_global_allyears_gadm2_non_geom.csv"))
...
new_col_names_gadm2_collapse_master_geom_ver <- colnames(aqli_gadm2_collapse_master_with_geom) %>%
  as_tibble() %>%
  mutate(col_names_shortened = str_replace(value, "llpp", "ll"), 
...
# making sure to st_as_sf the geom version before exporting to shape file
aqli_gadm2_collapse_master_with_geom %>%
  st_as_sf() %>%
  sf::write_sf(str_c(ssd_location_collapsed_gadm2_data_path, "master_global_allyears_gadm2_with_geom.shp"))
...
no_landmask_2018_processed_0.008 <- arrow::open_dataset("C:/Users/Aarsh/Desktop/aqli-epic/sat.data.processing/experimentation/others/2018_0.008_noLandMask/2018.parquet.no.landmask/part-0.parquet")
...
aqli_gadm2_collapse_na_summary_no_landmask_2018_proc_0.008 <- no_landmask_2018_processed_0.008 %>%
  dplyr::group_by(colormap_objectid) %>%
  dplyr::collect() %>%
  dplyr::summarise(count_rows = n(),
    total_nas_pol = sum(is.na(pm2.5_pollution)), 
