...
# setting the names of the newly created placheoldes
names(region_raster_brick) <- c("population_2021", "pollution_2021")
...
region_raster_brick_df %>% data.table::fwrite("./qgis_experimentation/polDataOnly.csv")

# benchmarking
threshold_6 <- Sys.time()

#> experimenting with postgres and sparklyr to 
...
dplyr::copy_to(con, region_raster_brick_df, "polDataOnly")

sc <- sparklyr::spar

region_raster_brick_df 

# read data in from postgres
polDataOnly_pg <- dplyr::tbl(con, "polDataOnly")

polDataOnly_pg_collect <- polDataOnly_pg %>%
...
polDataOnly <- spark_read_csv(sc, "./qgis_experimentation/polDataOnly.csv")

polDataOnly_spark <- polDataOnly %>% 
group_by(colormap_objectid) %>%
  dplyr::mutate(pop_weights_gadm2  = population_2021/sum(population_2021, na.rm = TRUE),
                pm2021_pop_weighted = pollution_2021*pop_weights_gadm2) %>%
    dplyr::summarise(avg_pm2.5_2021 = sum(pm2021_pop_weighted, na.rm = TRUE)) %>%
...
foo %>% 
  dplyr::group_by(colormap_objectid) %>% 
  arrow::write_dataset("./experimentation/arrow_r_test/foo_grouped.parquet")
  dplyr::collect() %>%
  dplyr::mutate(pop_weights = population_2021/sum(population_2021, na.rm = TRUE), 
                pm2021_pop_weighted = pop_weights * pollution_2021) %>% 
  dplyr::summarise(avg_pol = sum(pm2021_pop_weighted, na.rm = TRUE))
