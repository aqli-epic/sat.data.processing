...
foo_plt <- region_raster_brick_df_joined_colormap_summary_geom %>%
  ...
  ggthemes::theme_map() +
   labs(title = "Kenya: Life years lost relative to WHO guideline", 
...
region_raster_brick_df %>% data.table::fwrite("./qgis_experimentation/pol_0.01_2021_data.csv")

#> experimenting with postgres and sparklyr to l
...
sparklyr::spark_read_csv(sc, "pol_0.01_2021_data_with_gadm_columns", "./qgis_experimentation/pol_data_with_gadm_columns.csv")

# join with colormap (sans geometry)
region_raster_brick_df_joined_colormap <- region_raster_brick_df_test %>%
  left_join(colormap_region %>% st_drop_geometry(), by = c("colormap_objectid" = "objectid"))

data.table::fwrite(region_raster_brick_df_joined_colormap, "./qgis_experimentation/pol_0.01_2021_data_with_gadm_columns.csv")
...
# Christa, Random graph: China and Beijing LYL map at 0.01 resolution

```{r}
# crop colormap to the extent of Los Angeles
colormap_china <- colormap %>%
   filter(NAME_0 %in% "China") 

# cropping pollution 0.01 data to the extent of LA
pol_raw_0.01_china <- raster::crop(pol_raw_0.01, colormap_china)

# creating a life years lost layer
pol_raw_0.01_china_values <- values(pol_raw_0.01_china)
lyl_pol_raw_0.01_china <- (pol_raw_0.01_china_values - 5)*0.098
lyl_pol_raw_0.01_china_values <- ifelse(lyl_pol_raw_0.01_china < 0, 0, lyl_pol_raw_0.01_china)

pol_lyl_raw_0.01_china_brick <- pol_raw_0.01_china %>%
  addLayer(pol_raw_0.01_china)

names(pol_lyl_raw_0.01_china_brick) <- c("pm2021", "lyl_2021_rel_who")

values(pol_lyl_raw_0.01_china_brick$lyl_2021_rel_who) <- lyl_pol_raw_0.01_china_values

writeRaster(pol_lyl_raw_0.01_china_brick$lyl_2021_rel_who, "./experimentation/qgis/china_raster_0.01_lyl_2021.tif")
